...
- Keep KORION money display sources fixed by label. `mining-amount`, top-up availability, and `담보 전환 가능` must use `offline_pay` current snapshot `additionalCollateralAvailableAmount`; KORI `asset-balance` is `additionalCollateralAvailableAmount + spendableCollateralAmount`; Hub total collateral/담보예치금 should prefer server Hub projection `totalCollateralAmount`; offline spendable amount uses `offlineAvailableAmount` / `availableForPay` with local pending sends applied. Do not swap to raw wallet balances or stale local projection under the same UI label.
- For KORION money display fixes, "surgical" means fixing the shared balance basis owner, not adding one more selector fallback. If Hub, Dashboard, Flow, localStorage, native SQLite cache, and server summary disagree, define or repair one `userId + assetCode` balance basis/cache and make consumers read from it. Local ledger pending usage and optimistic operations must be explicit overlays on that basis, not alternate sources.
