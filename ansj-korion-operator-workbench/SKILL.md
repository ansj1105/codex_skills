...
- For KORION offline-pay/frontend balance issues, enforce fixed money-source mapping before coding: `mining-amount` / top-up available / `담보 전환 가능` = `additionalCollateralAvailableAmount`; KORI `asset-balance` = `additionalCollateralAvailableAmount + spendableCollateralAmount`; Hub total collateral/담보예치금 = server Hub projection `totalCollateralAmount`; offline spendable = `offlineAvailableAmount` / `availableForPay` with local pending sends applied.
