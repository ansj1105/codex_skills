...
- Current BLE baseline is working only when discovery/advertising, REQUEST ACK, receiver approval, sender PIN/biometric confirmation, COMPLETE ACK, and both completion screens pass. NFC work must preserve that baseline.
- NFC debugging should log tag start, native payload exchange/read/write, peer parse, authenticated peer commit, BLE bridge discovery/result, form transition, amount draft, actual REQUEST, sender confirm/auth, COMPLETE delivery, and cleanup as separate checkpoints.
