...
- For `fox_coin_frontend` Android release validation/builds, use the repo script `npm run android:release` when the user asks for an Android release build. On this workstation, the canonical build/install/log path is the Windows clone and Windows ADB, not WSL ADB:
  - Build from WSL through PowerShell after pulling the Windows clone: `powershell.exe -NoProfile -Command "Set-Location 'C:\work\fox_coin_frontend'; git pull --ff-only; npm run android:release"`.
  - APK path: `C:\work\fox_coin_frontend\android\app\build\outputs\apk\release\app-release.apk`.
  - Windows ADB path: `C:\Users\msi\AppData\Local\Android\Sdk\platform-tools\adb.exe`.
  - Current test devices: S23 `R3CWC06MHFP`, Flip `R3CT501EJHV`. Verify with `powershell.exe -NoProfile -Command "& 'C:\Users\msi\AppData\Local\Android\Sdk\platform-tools\adb.exe' devices -l"` before install/log capture.
  - PowerShell quoting guard: from WSL/bash, if `-Command` contains PowerShell variables such as `$adb`, `$apk`, `$job1`, or `$using:adb`, wrap the entire `-Command` value in single quotes or escape every `$`. Bad: `powershell.exe -NoProfile -Command "$adb='C:\...\adb.exe'; & $adb devices -l"` because bash expands `$adb` first. Good: `powershell.exe -NoProfile -Command '$adb="C:\Users\msi\AppData\Local\Android\Sdk\platform-tools\adb.exe"; & $adb devices -l'`.
  - Install both devices only after the build completes: use PowerShell `Start-Job` with `adb -s <serial> install -r <apk>`. Do not install an old APK or build without first pulling the Windows clone.
  - Logcat must use Windows ADB. Before a test, clear both buffers with `logcat -G 64M -c`; after the test, dump with `logcat -d -v time` and filter for `chromium`, `Capacitor/Console`, `OfflineBluetooth`, `OfflinePay`, `KORION`, `ReactNativeJS`, `AndroidRuntime`, `Minified React error`, `SETTLEMENT`, `outbox`, `ledger`, `snapshot`, `hub`.
  - App-local DB checks must use Windows ADB. Safe DB dump path: `run-as com.korion.wallet cp databases/offline_pay.db /sdcard/Android/data/com.korion.wallet/files/debug_export/<name>.db`, then `adb pull` that file. Do not pipe `exec-out cat databases/offline_pay.db` through PowerShell or WSL stdout; it can corrupt SQLite bytes or hit WSL `UtilBindVsockAnyPort` errors. WSL `adb devices` is usually empty here and should not be used for connected-device work.
  - If the pulled app-local DB must be inspected and neither `sqlite3` CLI nor Node built-in SQLite is available, use Python's standard `sqlite3` module for read-only queries. Open the DB with a read-only URI such as `file:/mnt/c/Temp/korion-db-check/s23-offline_pay.db?mode=ro`, do not write/repair/vacuum the file, and report that the query was read-only.
...
- Gradle checks in WSL should explicitly pin the workspace JDK instead of relying on shell PATH. For Java backend tests use:
  `env PATH=/home/ubuntu/work/jdk-21.0.7+6/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin JAVA_HOME=/home/ubuntu/work/jdk-21.0.7+6 ./gradlew test --tests <ClassOrPattern> --no-daemon`.
  For Android/Capacitor checks that need SDK tools, add `/home/ubuntu/work/android-sdk/platform-tools:/home/ubuntu/work/android-sdk/cmdline-tools/latest/bin` to that PATH. If Gradle says it cannot find `java`, fix the command environment first; do not treat it as a project failure.
...
- For app-local offline-pay state, treat `deviceId + userId` as the active owner boundary. When the logged-in owner changes, active local DB/session/projection/cache may be reset after pausing local workers. Preserve previous-owner evidence by archive/rotation when needed; do not keep prior-user rows in the new user's active DB just to avoid deletion.
...
- `korion_offline` uses Spring scheduling (`@EnableScheduling`) in `app-api`/`app-worker`. Worker knobs live under `offline-pay.worker.*` in `src/main/resources/application.yml`.
- `korion_offline` settlement workers include `SettlementWorker`, `SettlementExternalSyncWorker`, `CollateralExternalSyncWorker`, `ReconciliationFollowUpWorker`, `OfflineWorkflowProjector`, `DirectLocalEvidenceReconciliationWorker`, `OfflineEventLogMaintenanceWorker`, `PostFinalProofConflictScanWorker`, `CollateralBalanceReconciliationWorker`, and `IssuedProofApplicationService` collateral device sync. Orphan received-unsettled cleanup is `OrphanReceivedUnsettledCleanupWorker` and runs on `offline-pay.worker.orphan-received-unsettled-cleanup-cron` (default `0 20 4 * * *`, UTC unless overridden).
- `coin_manage` splits long-running work by container. `korion-app-withdraw-worker-1` owns BullMQ withdraw `dispatch`, `reconcile`, and `external_sync` jobs. `korion-app-ops-1` owns singleton operational workers such as deposit monitor, sweep bot, monitoring, alerts, outbox publisher, offline-pay ledger reconciliation, activation grant/reclaim, and resource delegation.
- `coin_manage` manual scheduler HTTP routes are under `/api/scheduler`, including `POST /api/scheduler/process-withdraw-queue` and `POST /api/scheduler/retry-pending`. Use these for operator-triggered recovery rather than inventing ad-hoc scripts.
- `fox_coin_frontend` mobile offline-pay retry is local durable outbox state (`offline_pay_sync_outbox` and attempts), not the legacy `offline_pay_queue`. Queue rows are compatibility/protocol recovery state only; server upload/retry ownership belongs to the outbox worker.
- `alarm_service` is an external Python monitor loop (`POLL_INTERVAL_SECONDS`, default 30s). It watches web/API/PostgreSQL/container/log health and sends Telegram only after configured consecutive failures; it is not the offline-pay settlement worker.
...
| `kori_hompage` | KORION 공식 홈페이지 | `100.50.107.232` | `/var/www/korion` | React/Vite + Node.js |
...
| `alarm_service` | KORION web/API/DB Telegram 알람 | `52.200.97.155` | — | Python |

### SSH 키
- KORION 전체 서비스: `~/.ssh/korion.pem` (Windows: `C:\work\korion.pem`)
- 신호텍: `C:\work\shinotek.pem`

### 모니터링
- Grafana: `https://dev.korion.io.kr/`
- Prometheus: `https://api.korion.io.kr/prometheus/`
- Web 헬스: `https://korion.io.kr/health`, API 헬스: `https://api.korion.io.kr/health`
...
# fox_coin_service (52.200.97.155) — prod compose 필수
...
# kori_hompage (100.50.107.232)
sudo git pull && sudo ./scripts/deploy-compose.sh
